  ...
  var chartData = theData;
   ...
   function generateStarBurst(json) {
     ...
     var technologyText = vis.selectAll("technologyText")
       ...
       .style("font-size", function (d) { return (d.depth == 2) ? "9px" : "10px"; })
       .style("font-weight", "bold")
       .style("fill", function (d) { return (d.depth == 2) ? colors[d.parent.name] : ""; }) //Colors of names around the circle
       .attr("dx", function (d) { return (d.depth == 1) ? 75 : 300; }) // margin
       .attr("dy", ".35em") // vertical-align
       .text(function (d) { return d.depth == 2 ? "-" + d.Title.toUpperCase() : ""; })
       .on("mouseover", mouseover)
   ...
   function getTechCircleData(d, i) {
     if (d.Year <= startYear) {
       ...
       return arc.centroid(d)[i] * (0.17 + 0.0132 * (d.Year - baseYear + 1)); //Year
     }
   }
 
   function getTechnologyIndex(d) {
    //  if (d.depth == 2) {
    //    return d.parent.children.map(function (e) { return e.name; }).indexOf(d.name) + 2;
    //  }
   }
 
 });
 
>>>>>>> fb9f61fb0ce324619b475cdcf228795dc094571b
